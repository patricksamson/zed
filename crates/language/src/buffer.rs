...
pub struct Buffer {
    ...
    /// Filesystem state, `None` when there is no filesystem path.
...
/// State of file persistence.
///
/// `New` and `Deleted` are quite similar as they both correspond to a
        ...
        if let Some(old_file) = self.file.as_ref() {
            ...
            if old_state != new_state {
                ...
                match (old_state, new_state) {
                    (
                        DiskState::Present { mtime: old_mtime },
                        DiskState::Present { mtime: new_mtime },
    ...
    pub fn has_conflict(&self) -> bool {
        ...
        match file.disk_state() {
            ...
            DiskState::Present { mtime } => match self.saved_mtime {
                // mtimes are checked with (!=) rather than (>) because they can become older.
